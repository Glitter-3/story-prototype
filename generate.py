...
from PIL import Image  # 新增：用于图片尺寸处理
...
def check_and_adjust_image_size(image_paths):
    ...
    try:
        ...
        with Image.open(image_paths[0]) as base_img:
            ...
            print(f"📏 基准图片尺寸: {base_size[0]}x{base_size[1]} (宽x高)")
            ...
            try:
                with Image.open(img_path) as img:
                    ...
                    if current_size == base_size:
                        print(f"✅ 图片 {os.path.basename(img_path)} 尺寸正确: {current_size[0]}x{current_size[1]}")
                        adjusted_paths.append(img_path)
                        continue
                    
                    # 尺寸不一致，进行缩放调整
                    print(f"🔄 调整图片 {os.path.basename(img_path)} 尺寸: {current_size[0]}x{current_size[1]} -> {base_size[0]}x{base_size[1]}")
                    ...
                    print(f"✅ 已调整并保存: {temp_path}")
                        
            except Exception as e:
                print(f"❌ 处理图片 {img_path} 时出错: {e}")
                adjusted_paths.append(img_path)  # 出错时使用原图片
        
        # 如果所有图片都无需调整，清理临时目录
        if len(adjusted_paths) == len(image_paths) and all(p in image_paths for p in adjusted_paths):
            shutil.rmtree(temp_dir, ignore_errors=True)
            print("🗑️ 无需调整图片，已清理临时目录")
        else:
            print(f"📁 调整后的图片保存在: {temp_dir}")
